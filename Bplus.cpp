...
int main() 
{
    ...
    while (true)
    {
        ...
        if(op==1)
        {
            ...
            while (now->leaf==0)
            {  
                // in this while loop we go to the appropriate children till we reach leaf
                if(now->data[0]>val)
                {
                    // in case we need to go to the leftmost child
                    now=now->children[0];
                }
                else if(val>=now->data.back())
                {
                    // in case we need to go to the rightmost child
                    now=now->children.back();
                }
                else
                {
                    // in case any middle node
            ...
            {   
                // if after insertion there is no overflow of leaf
                continue;
            }
            else
            {
                ...
                { 
                    // we are pushing overflow towards root, if it is fixed in mid then well and good else we create a new root
                    ...
                    {
                         // if my parent size after sibling is greater than 2t+1 then split my parent and give them till "now" and "sibling"
                        index_count++;
                        sibling = new block(0);
                        ...
                        for(int i=0;i<t+2;i++)
                        { 
                            // last t+2 children pointer in sibling
                ...
                if(!flg)
                { 
                    // if we got till root and still splitted , then we create a new root
